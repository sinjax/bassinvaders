...
int BandPassFilterFFT::ffind(double freq){
	return gsl_interp_bsearch(f, freq, 1, freqs);//gsl_interp_accel_find (faccel, f, freqs, freq );
}
...
{
	band_window(fcache, stream, ffind(fhi)+1, ffind(flo));
}

/*
 * using the frequency fft data, band pass between frequencies in f[bandlo] <= f <= f[bandhi]
 */
void BandPassFilterFFT::band_window(fft_t data, uint8_t * stream, uint32_t bandhi, uint32_t bandlo)
{
	/*
